...
MODULE interpolation
  ...
  SUBROUTINE bilinear(x_axis, len_x, y_axis, len_y, data_in, &
    ...
    double precision, DIMENSION(len_x) :: x_axis
    double precision, DIMENSION(len_y) :: y_axis
    double precision, DIMENSION(len_y, len_x) :: data_in
    double precision, DIMENSION(num_points, 2) :: points ! (# of pts, 2)
    double precision, DIMENSION(num_points) :: data_ip
    ...
    double precision :: dx_inv, dy_inv
    INTEGER :: ind_x, ind_y
    double precision :: value_ofb = -9999.0  ! Out-of-bounds value
    double precision :: w_11, w_12, w_21, w_22
    ...
    END DO
    dx_inv = 1.0 / (delta / dble(len_x - 1))
    delta = 0.0
    DO ind = 1, (len_y - 1)
      delta = delta + (y_axis(ind + 1) - y_axis(ind))
    END DO
    dy_inv = 1.0 / (delta / dble(len_y - 1))
  ...
  SUBROUTINE idw_kdtree(points, num_points, data_in, &
    ...
    double precision, DIMENSION(2, num_points) :: points ! (2, # of pts)
    double precision, DIMENSION(num_points) :: data_in
    double precision, DIMENSION(len_x) :: x_axis
    double precision, DIMENSION(len_y) :: y_axis
    INTEGER :: num_neighbours
    double precision, DIMENSION(len_y, len_x) :: data_ip
    ...
    INTEGER :: i
    INTEGER :: ind_x, ind_y
    double precision, DIMENSION(2) :: point_target
    double precision, DIMENSION(:, :), allocatable :: neighbours
    INTEGER, DIMENSION(:), allocatable :: neighbours_index
    double precision :: numerator, denominator, dist
  ...
  SUBROUTINE idw_esrg_nearest(points, num_points, data_in, &
    ...
    double precision, DIMENSION(:), allocatable :: dist_nn
  ...
  SUBROUTINE idw_esrg_connected(points, num_points, data_in, &
    ...
    INTEGER, DIMENSION(:), allocatable :: index_nn
    double precision, DIMENSION(:), allocatable :: dist_nn
    double precision, DIMENSION(:) :: centre(2)
