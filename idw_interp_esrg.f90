...
MODULE idw_interp_esrg
  ...
  SUBROUTINE assign_points_to_cells(points, x_axis, y_axis, grid_spac, &
    ...
    double precision, DIMENSION(:, :), INTENT(IN) :: points ! (# of pts, 2)
    double precision, DIMENSION(:), INTENT(IN) :: x_axis, y_axis
    double precision, INTENT(IN) :: grid_spac
  ...
  SUBROUTINE nearest_neighbours_esrg(points, &
    ...
    double precision, DIMENSION(:), INTENT(OUT) :: dist_nn

    INTEGER :: len_x, len_y
    double precision :: radius_sq, dist_sq
    double precision, DIMENSION(:) :: dist_sq_nn(num_nn)
    INTEGER :: i, j, k
    INTEGER :: ind_x_nb, ind_y_nb
    INTEGER :: ind, ind_is, level, num_nn_found
    double precision, DIMENSION(:) :: centre(2)
    TYPE(point_attr), DIMENSION(50000) :: points_cons
    INTEGER :: num_pts_cons
    TYPE(point_attr), DIMENSION(50000) :: points_cons_next
