extern void stackinit_(int *newsize);
...
void stackpairinit_(int *size) {
    if (s_stack.stack) return;  // stack already initialized
...
static void growstack_() {
    ...
    printf(
        "Growing stack to %d... Consider using a bigger stack right from the "
        "start.\n",
        newsize);
    stackinit_(&newsize);
...
void pushpair_(int *obja, int *objb) {
    ...
    if (s_stack.ptr == s_stack.end) growstack_();  // this is expensive, avoid
...
void stackpairflush_() {
    if (s_stack.ptr == s_stack.stack) return;
