...
float cvolume(float *a, float *b, int *m, int *n, int *mmax, int *nmax) {
    ...
    if (nn == 1) {
        ...
        for (l = 0; l < mm; l++) {
            ...
            {
                /*       printf("Inconsistent constraints found after reduction
                 * to n = 1 \n");*/
                return (0.);
            }
        }
        ...
        if (firstmin * firstmax == 1)
            ...
            /*     printf("Volume is unbounded at 1-D; volume returned as
             * -1\n");*/
            return (-1.0);
        }
    ...
    for (i = 0; i < mm; i++) {
        ...
        if (amax == 0.) {
            ...
            /* reduce a and b into ap and bp eliminating variable t and
             * constraint i */
                    ...
                    *(bp + jj) = *(b + j) - *(bi) * *(ajt) / pivot;
                    ...
                    for (k = 0; k < nn; k++)
                        if (k != t) {
                            ...
                            kmm = k * mm_max;
                            *(apjj + kk * mm1) =
                                *(aj + kmm) - *(ajt) * *(ai + kmm) / pivot;
                        }
                }

            /* add contribution to volume from volume calculated in smaller
             * dimension */
...
void volume_(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
             float *volume) {
...
float cvolumeb(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
               int *opt, float *dvdb) {
    float v, amax, pivot;
    int i, j, t, k, l;
    int jj, kk;
    ...
    if (nn == 1) {
        ...
        *dvdb = 0.;
        if (firstmin * firstmax == 1)
            ...
            printf(
                "Volume is unbounded; volume returned as -1\n derivatives "
                "returned as zero\n");
            return (-1.0);
        }
...
void volumeb_(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
              int *opt, float *volume, float *dvdb) {
                               ...
                               but not volume */
    {
        *volume = cvolumeb(a, b, m, n, mmax, nmax, opt, dvdb);
        ...
        printf(
            " Warning: routine volumeb called with an invalid option "
            "parameter\n");
...
float cvolumebj(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
                int con, float *dvdb) {
    float v, amax, pivot;
    int i, j, t, k, l;
    int jj, kk;
    ...
    if (nn == 1) {
        ...
        *dvdb = 0.;
        ...
        if (v < 0.) {
            v = 0.;
            *dvdb = 0.;
        } else if (v > 0. && lmin == con)
            *dvdb = 1. / *(a + lmin);
        else if (v > 0. && lmax == con)
            *dvdb = -1. / *(a + lmax);
        return (v);
    }
...
void volumebj_(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
               int *con, float *volume, float *dvdb) {
    int tcon;
    tcon = *con - 1;
...
float cdvda(float *a, float *b, int *m, int *n, int *mmax, int *nmax, int *tdim,
            float *temp, int *jval, int *code) {
    float v, amax, pivot;
    int i, j, t, k, l;
    int jj, kk;
    float *ai, *aj, *ajt, *apjj, *bi;
    ...
    *code = 0;
    ...
    for (i = 0; i < mm; i++) {
        ...
        if (amax != 0) {
            ...
            } else { /* Constraint 0 has not yet been
                         encountered */
                jjval = 1;
            }
            ...
            /* add contribution to derivative from that calculated in smaller
             * dimension */

            /* Normal case method */
            if (special == 0) {
                deriv = cdvda(ap, bp, &mm1, &nm1, &mm1, &nm1, &ttdim, ttemp,
                              &jjval, code);
            ...
            } else /* Use special case method */
            {
                if (*jval == 1) {
                    if (i == 0) /* This is constraint 0 */
                    {
                        ...
                        for (j = 1; j < mm; j++) {
                            k = j - 1;
                            junk = cvolumebj(ap, bp, &mm1, &nm1, &mm1, &nm1, k,
                                             &dvdb);
                            if (junk == -1.) {
                                *code = -1;
                                return (0.);
                    ...
                    {
                        opt = 2;
                        junk = cvolumeb(ap, bp, &mm1, &nm1, &mm1, &nm1, &opt,
                                        &deriv);
                        if (junk == -1.) {
                            *code = -1;
                            return (0.);
                ...
                } else if (*jval == 0) /* Constraint 0 already encountered */
                {
                    ...
                    for (j = 0; j < i; j++) {
                        junk =
                            cvolumebj(ap, bp, &mm1, &nm1, &mm1, &nm1, j, &dvdb);
                        if (junk == -1.) {
                            *code = -1;
                            return (0.);
                        }
                        dbda = (*(a + j + tmm) * *(temp + i) / pivot) -
                               *(temp + j);
                    ...
                    for (j = i + 1; j < mm; j++) {
                        k = j - 1;
                        junk =
                            cvolumebj(ap, bp, &mm1, &nm1, &mm1, &nm1, k, &dvdb);
                        if (junk == -1.) {
                            *code = -1;
                            return (0.);
                        }
...
void dvda_(float *a, float *b, int *m, int *n, int *mmax, int *nmax, int *idim,
           float *dvda, int *code) {
    int j;
    int jval, tdim;
    float *temp;
...
void volumef_(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
              float *volume) {
...
float cdvdaf(float *a, float *b, int *m, int *n, int *mmax, int *nmax,
             int *tdim, float *temp, int *jval, int *code) {
    float v, amax, pivot;
    int i, j, t, k, l;
    int jj, kk;
    float *ai, *aj, *ajt, *apjj, *bi;
    ...
    if (nn == 1) {
        ...
        for (l = 0; l < mm; l++) {
            if (*(a + l) > 0.) {
                ...
                /*printf("Inconsistent constraints found after reduction to n =
                 * 1 \n");*/
                *code = 1;
                return (0.);
            }
    ...
    for (i = 0; i < mm; i++) {
        ...
        if (amax != 0) {
            ...
            if (special == 0) {
                deriv = 0.;
                if (*(bi) != 0.) {
                    deriv = cdvdaf(ap, bp, &mm1, &nm1, &mm1, &nm1, &ttdim,
                                   ttemp, &jjval, code);
                }
                v = v + *(bi) / amax * deriv / nn;
                if (kval == 1 || *jval == 0) free(ttemp);
...
void dvdaf_(float *a, float *b, int *m, int *n, int *mmax, int *nmax, int *idim,
            float *dvda, int *code) {
    int j;
    int jval, tdim;
    float *temp;
...
float cdvda_debug(float *a, float *b, int *m, int *n, int *tdim, float *temp,
                  int *jval, int *code) {
    float v, amax, pivot;
    int i, j, t, k, l;
    int jj, kk;
    ...
    for (i = 0; i < mm; i++) {
        ...
        if (amax == 0.) {
            if (*(ai + ttdim * mm) == 0.0) {
                ...
                printf(
                    "Constraint %d can only be projected onto dimension %d\n",
                    i + 1, *tdim);
            ...
            } else /* Use special case method */
            {
                if (*jval == 1) {
                    if (i == 0) /* This is constraint 0 */
                    {
                        ...
                        dvdb = 0.;
                        printf(
                            "\n special case 2: reduction by 0 and variable "
                            "k\n");
                        for (j = 1; j < mm; j++) {
                            k = j - 1;
                            junk = cvolumebj(ap, bp, &mm1, &nm1, &mm1, &nm1, k,
                    ...
                    {
                        printf(
                            "\n special case 1: constraint 0 not yet "
                            "reached\n");
                        opt = 2;
                        junk = cvolumeb(ap, bp, &mm1, &nm1, &mm1, &nm1, &opt,
                                        &deriv);
                        ...
                        deriv = -(deriv * *(bi) / pivot);
                        v = v + *(bi) / amax * deriv / nn;
                        printf("\n spec: 0 not yet reached i %d dcont %f", i,
                               deriv);

                        /* debug section: reduce system and repeat calculation
                ...
                } else if (*jval == 0) /* Constraint 0 already encountered */
                {
                    ...
                    deriv = (*(bi)*deriv - *(temp + i) * vol) / pivot;
                    v = v + *(bi) / amax * deriv / nn;
                    printf("\n spec: 0 already encountered i = %d dcont %f", i,
                           deriv);
                }
            }
